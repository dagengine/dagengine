...
export class TavilyProvider extends BaseProvider {
    ...
    async execute(request: ProviderRequest): Promise<ProviderResponse<TavilyResult[]>> {
        try {
            const queries = Array.isArray(request.input) ? request.input : [request.input];
            const maxResults = (request.options?.maxResults as number) || 5;
            const searchDepth = (request.options?.searchDepth as string) || 'advanced';
