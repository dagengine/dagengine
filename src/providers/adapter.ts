import { BaseProvider, ProviderRequest, ProviderResponse } from './types';
...
export class ProviderAdapter {
  ...
  private initializeProviders(config: ProviderAdapterConfig): void {
    ...
    if (config.anthropic) {
      if (!config.anthropic.apiKey) {
        console.warn('Anthropic provider config provided but API key is missing');
      } else {
        try {
    ...
    if (config.whoisxml) {
      if (!config.whoisxml.apiKey) {
        console.warn('WhoisXML provider config provided but API key is missing');
      } else {
        try {
   ...
   */
  async execute(providerName: string, request: ProviderRequest): Promise<ProviderResponse> {
