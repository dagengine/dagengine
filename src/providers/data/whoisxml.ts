...
export class WhoisXMLProvider extends BaseProvider {
    ...
    private readonly cache = new Map<string, { data: WhoisData; timestamp: number }>();
    private readonly cacheTTL: number;

    constructor(config: ProviderConfig) {
        super('whoisxml', 'data', config);
    ...
    async execute(request: ProviderRequest): Promise<ProviderResponse<WhoisData[]>> {
        try {
            const domains = Array.isArray(request.input) ? request.input : [request.input];
