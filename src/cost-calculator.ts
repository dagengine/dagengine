...
export class CostCalculator {
    ...
    calculate(
        ...
        for (const { results } of sectionResults) {
            for (const [dimension, result] of Object.entries(results)) {
                const cost = this.processDimensionResult(dimension, result, byDimension, byProvider);
                if (cost !== null) {
    ...
    private processDimensionResult(
        ...
        byProvider: Record<string, { cost: number; tokens: TokenUsage; models: string[] }>
        ...
        this.updateDimensionCost(byDimension, dimension, cost, tokens, model, provider);
