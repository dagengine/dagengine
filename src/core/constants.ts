...
export const ERROR_MESSAGES = {
    ...
    DEPENDENCIES_FAILED: (dimension: string) => `Dependencies failed for dimension "${dimension}"`,
    TIMEOUT: (dimension: string, timeout: number) =>
        `Timeout after ${timeout}ms for dimension "${dimension}"`,
    DEPENDENCY_NOT_FOUND: (depName: string) => `Dependency "${depName}" not found in plugin dimensions`,
    GLOBAL_DEP_NOT_FOUND: (depName: string) => `Global dependency "${depName}" not found`,
    SECTION_DEP_NOT_FOUND: (depName: string) => `Section dependency "${depName}" not found`,
    SECTION_DEP_NOT_PROCESSED: (depName: string) => `Section dependency "${depName}" not yet processed`,
} as const;
