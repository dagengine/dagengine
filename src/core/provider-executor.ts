...
export class ProviderExecutor {
    ...
    async execute(
        ...
        // Create prompt
        const prompt = await this.createPrompt(dimension, sections, dependencies, isGlobal);
    ...
    private async selectProvider(dimension: string, section: SectionData) {
        return await Promise.resolve(this.plugin.selectProvider(dimension, section));
    }
    ...
    private async handleRetry(
        ...
        const result: { shouldContinue: boolean; modifiedRequest?: ProviderRequest } = {
            shouldContinue: true,
        };
    ...
    private async handleFallback(
        ...
        const fallbackResponse = await this.hookExecutor.handleFallback(fallbackContext);
