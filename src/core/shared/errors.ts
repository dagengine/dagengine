...
export class NoProvidersError extends ConfigurationError {
    constructor(message: string = 'DagEngine requires at least one provider to be configured') {
...
export class NoSectionsError extends ConfigurationError {
    constructor(message: string = 'DagEngine.process() requires at least one section') {
...
export class DependencyError extends DagEngineError {
    ...
    constructor(dimension: string, failedDeps: Record<string, string>) {
        ...
        const message = failedDeps && Object.keys(failedDeps).length > 0
        ...
        this.failedDependencies = Object.entries(failedDeps).map(([name, error]) => ({
            name,
            error
        }));
    }
