...
import { PhaseExecutor } from '../core/engine/phase-executor';
import { createProcessState, serializeState, deserializeState } from '../core/engine/state-manager';
        ...
        return this.inngest.createFunction(
            ...
            async ({event, step}: any) => {
                ...
                const plan = await step.run('plan-execution', async () => {
                    const deserializedState = deserializeState(state);
                    return await this.phaseExecutor.planExecution(deserializedState, options);
                });
