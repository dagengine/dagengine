...
import { computed, shallowRef, toValue, watch } from "./chunk-XV6CE6JT.js";
...
var createFocusTrap = function createFocusTrap2(elements, userOptions) {
		...
		if (targetContained || target instanceof Document) {
			...
			if (state.mostRecentlyFocusedNode) {
				if (getTabIndex(state.mostRecentlyFocusedNode) > 0) {
					...
					if (
						!state.containerGroups.some((g) => g.tabbableNodes.some((n) => getTabIndex(n) > 0))
					) {
						navAcrossContainers = false;
					}
				}
	...
	trap = {
		...
		updateContainerElements: function updateContainerElements(
			...
			state.containers = elementsAsArray.map((element) => typeof element === "string"
