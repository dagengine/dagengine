...
describe('shouldSkipDimension - Edge Cases', () => {
    ...
    test('should handle when some sections skip, some do not', async () => {
        ...
        expect(result.sections[0]?.results.process?.data).not.toHaveProperty('skipped');
        expect(result.sections[1]?.results.process?.data).toEqual({
            skipped: true,
            reason: 'Skipped by plugin shouldSkipDimension',
        });
        expect(result.sections[2]?.results.process?.data).not.toHaveProperty('skipped');
    ...
    test('should handle skip logic with complex metadata structures', async () => {
        class ComplexMetadataPlugin extends Plugin {
            constructor() {
                super('complex-metadata', 'Complex Metadata', 'Handle complex metadata');
                this.dimensions = ['process'];
            }
        ...
        const result = await engine.process([
            { content: 'Section 1', metadata: { config: { processing: { skip: true } } } },
            { content: 'Section 2', metadata: { config: { processing: { skip: false } } } },
...
describe('shouldSkipGlobalDimension - Edge Cases', () => {
    ...
    test('should handle global dimension skip with parallel global group', async () => {
        class ParallelGlobalSkipPlugin extends Plugin {
            constructor() {
                super('parallel-global', 'Parallel Global', 'Parallel global dimensions');
                this.dimensions = [
    ...
    test('should handle complex aggregate conditions for global skip', async () => {
        class ComplexAggregatePlugin extends Plugin {
            ...
            shouldSkipGlobalDimension(context: SectionDimensionContext): boolean {
                ...
                const hasImportant = sections.some(s => s.content.includes('important'));
                return !hasImportant;
            }
        }
