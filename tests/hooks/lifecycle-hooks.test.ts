    ...
    describe('beforeProcessStart', () => {
        ...
        test('should handle async beforeProcessStart', async () => {
            ...
            class AsyncBeforePlugin extends TestPlugin {
                ...
                async beforeProcessStart(context: BeforeProcessStartContext): Promise<ProcessStartResult> {
    ...
    describe('handleProcessFailure', () => {
        test('should be called when process fails', async () => {
            ...
            class FailurePlugin extends TestPlugin {
                ...
                handleProcessFailure(context: ProcessFailureContext): ProcessResult | null {
                    hookCalled = true;
                    capturedContext = context;
                    return null;
                }
            ...
            await expect(engine.process([{ content: 'Test', metadata: {} }])).rejects.toThrow();
        ...
        test('should recover from process failure', async () => {
                ...
                handleProcessFailure(context: ProcessFailureContext): ProcessResult {
                    ...
                    return {
                        sections: context.sections.map(s => ({
                            ...
                            results: {
                                failing: {
                                    data: { recovered: true, originalError: context.error.message },
            ...
            expect(result.sections[0]?.results.failing?.metadata?.recoveredFromError).toBe(true);
        });
        ...
        test('should handle async handleProcessFailure', async () => {
            ...
            class AsyncFailurePlugin extends TestPlugin {
                ...
                async handleProcessFailure(context: ProcessFailureContext): Promise<ProcessResult> {
                    await new Promise(resolve => setTimeout(resolve, 50));
                    asyncCompleted = true;
